...
import { generatePose, transcribeAudio, translateSignWriting } from './api/client';
...
function App() {
  ...
  const runTranslation = async (text: string) => {
    ...
    try {
      ...
      if (tokens.length > 0) {
        const blob = await generatePose(text);
        setPoseFile(blob);
      }
    } catch {
  ...
  return (
    <div className="min-h-screen transition-all duration-300">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="xl:col-span-5 h-full">
            <div className="card h-full flex flex-col">
              <div className="pb-4 border-b border-theme-primary">
                ...
                <p className="text-sm text-theme-secondary">Type or speak your message to translate</p>
              </div>
                ...
                {isGeneratingAnimation ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-theme-secondary">Generating animation...</p>
                    </div>
                  </div>
                ) : (
        ...
        {error && (
          <div className="mt-6 bg-danger-50 border border-danger-200 rounded-lg p-4 text-danger-800">{error}</div>
        )}
